...
#define META_NAME "RR30MTA"
#define DATA_PREFIX "RR30D"
 ...
 * this release; RICKROLL uses the verified standard ZX7 core instead. */
extern void rickroll_zx7_decompress(void *destination, const void *source);
...
static void show_error(const char *line1, const char *line2) {
    ...
    gfx_PrintStringXY("RICKROLL ERROR", 16, 88);
...
static bool load_metadata(rickroll_metadata_t *result) {
    uint8_t handle = ti_Open(META_NAME, "r");
    if (!handle) {
        show_error("Metadata RR30MTA is missing.", "Install the complete RICKROLL package.");
        return false;
    }
    ...
    if (size < 21 || size > sizeof(metadata_buffer)
        ...
        ti_Close(handle);
        show_error("Metadata is truncated.", "RICKROLL stopped before playback.");
        return false;
    }
    ti_Close(handle);

    if (memcmp(metadata_buffer, "RR30", 4) != 0
        || metadata_buffer[4] != 1
        ...
        show_error("Metadata format is not supported.", "RICKROLL stopped before playback.");
        return false;
    }
    ...
    if (result->total_frames == 0
        ...
        show_error("Metadata integrity check failed.", "RICKROLL stopped before playback.");
        return false;
    }
    ...
    for (uint16_t segment = 0; segment < result->segment_count; ++segment) {
        ...
        if (frames == 0 || frames > FRAMES_PER_SEGMENT
            ...
            show_error("Metadata segment record is invalid.", "RICKROLL stopped before playback.");
            return false;
        }

        if (segment + 1 < result->segment_count && frames != FRAMES_PER_SEGMENT) {
            show_error("A non-final segment is short.", "RICKROLL stopped before playback.");
            return false;
        }
    }
...
static bool load_segment(uint16_t segment) {
    ...
    if (!handle) {
        show_error("A RICKROLL data segment is missing.", name);
        return false;
    }
    ...
    if (ti_GetSize(handle) != compressed_size
        ...
        ti_Close(handle);
        show_error("A RICKROLL data segment is truncated.", name);
        return false;
    }
    ti_Close(handle);

    rickroll_zx7_decompress(segment_buffer, compressed_buffer);
    if (crc16_ccitt(segment_buffer, raw_size) != expected_crc) {
        show_error("A RICKROLL data segment is corrupt.", name);
        return false;
    }
...
static void show_debug_statistics(uint32_t displayed_frames) {
    ...
    memset((void *)gfx_vbuffer, PALETTE_WHITE, GFX_LCD_WIDTH * GFX_LCD_HEIGHT);
    gfx_SetTextFGColor(PALETTE_BLACK);
    gfx_PrintStringXY("RICKROLL DEBUG", 16, 88);
